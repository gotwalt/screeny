...
rollback-capable bootloader and two OTA slots. UDP conformance on 0.5.0 is green (60/0/4);
pending: the phone test (see card 223). Next: 229 (network scan list), 230/231 (the
...
| 223 | **done, on the device (fw 0.5.0)** - HTTP suite green with the 223 rules enforced; the 64-rule UDP suite 60/0/4 (re-run 2026-09-20 with the Mac's WiFi off); **pending: the phone test with the owner** (steps and build command in the card's Log). Scanning split out to 229. Was: firmware: APSTA soft-AP, DHCP, DNS catch-all, the portal state machine wired to the store, the portal screen, the settings page (scan list, trial join) | yes |
