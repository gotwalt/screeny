...
**Status (2026-09-20, night): the device runs fw 0.7.0, with working over-the-air updates - settings and WiFi
...
| 246 | follow-ups from the OTA bench: `busy` after a confirmed update until reboot; the probe's wait-for-reboot; whose `fw_state` it is after a revert; the probe's restore guard on a broken pipe | yes |
