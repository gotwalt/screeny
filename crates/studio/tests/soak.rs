...
async fn the_server_survives_a_bounded_soak() {
    ...
    let set = post(at, "/api/v1/player/set", &format!(r#"{{"device":"{DEVICE}","patch":"metaballs","seed":1,"fps":30}}"#)).await;
    ...
    // patch, the pipeline, the encoder's tables and the link's buffers, and
    ...
    while Instant::now() < deadline {
        ...
        match round % 4 {
            ...
            //    keeps its id, and therefore its player and its patch.
            3 => {
                drop(sim);
                ...
                assert_eq!(d["player"]["patch"], "metaballs", "a panel that moved is still playing the same thing: {d}");
            }
            ...
            _ => {
                for (patch, fps) in [("metaballs", 60), ("clocks-numerals", 30), ("clocks-dials", 10)] {
                ...
                post(at, "/api/v1/player/set", &format!(r#"{{"device":"{DEVICE}","patch":"metaballs","fps":30}}"#)).await;
