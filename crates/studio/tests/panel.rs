...
use common::{get, post, preview_of, seq_of, studio, Ws};
...
/// Card 105's half of card 101's acceptance: the studio streams over the
/// network because a browser asked it to, and what the device puts up is what
/// the browser is drawing.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn send_to_panel_streams_the_preview_to_the_device() {
    ...
    assert_eq!(first.json()["target"], format!("127.0.0.1:{port}"));
    ...
    assert_eq!(off.json(), serde_json::Value::Null);
