...
//!
//! Everything here is integer-only and wrapping, exactly like the firmware's:
//! the counters are `u32` and wrap, and the EWMAs are `u16` and saturate. A
//! host has no trouble with `f64`, but the point of the simulator is to be
//! indistinguishable from the device, and a float would quietly disagree with
//! it in the last digit.
...
impl Counters {
    /// Add one, wrapping, like the device's `u32` counters do.
...
impl Timings {
    /// Record one decode.
    pub fn decode(&mut self, us: u64) {
        let us = us.min(u16::MAX as u64) as u16;
