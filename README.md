...
| `crates/fwimage` | `no_std` image validator: what makes an ESP32 app image one this device will run (research 006 section 5). Linked by the firmware, the simulator and the probe, so all three refuse the same uploads. |
| `firmware/` | The ESP32 firmware (separate cargo project, `esp` toolchain). |
| `crates/art` | The generative art system: patches, panel-aware pipeline, headless `screeny-art` binary. The primary source of frames. |
