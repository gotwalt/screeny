...
use super::{make_data_template, map_index, Entry, INTER_ROW_BLANK, OE_ACTIVE, OE_BLANK};
use crate::{LEAD_BLANK_DELAY, TRAIL_BLANK_DELAY};
    ...
    DmaFrameBuffer<NROWS, COLS, PLANES>
{
    ...
    pub const fn format(&mut self) {
        ...
        while p < PLANES {
            ...
            while slot < NROWS {
                ...
                self.planes[p].rows[slot].format(prev_addr);
                slot += 1;
            }
            #[cfg(feature = "tail-closes-latch")]
            {
    ...
    // -----------------------------------------------------------------
    // screeny local patch — see firmware/vendor/README.md
    // -----------------------------------------------------------------

    /// The widest output-enable window this build can produce, in pixel-clock
    /// slots per scan row.
    ///
    /// `make_data_template` lights slots `TRAIL_BLANK_DELAY ..
    /// COLS - LEAD_BLANK_DELAY - 1`; the last slot always carries the latch
    /// and is always blank. This is the number of slots in that window, and
    /// therefore the denominator of the duty cycle: full brightness is
    ...
    ///
    /// This is the mechanism Tidbyt's own firmware calls `setBrightness8()`.
    /// It rewrites only bit 8 (OE) of every entry, leaving colour data, row
    /// address and latch untouched, so **it costs no colour depth**: all
    /// `PLANES` bit-planes keep their full range and their relative weights,
    /// the whole panel simply spends less time lit. It also does not change
    /// the length of the DMA stream, so the refresh rate is unaffected.
    ///
    /// `lit` is clamped to [`Self::OE_SLOTS`]. Zero blanks the panel.
    ///
    /// Call it on a buffer you own (not the one DMA is currently streaming),
    /// or accept one frame of tearing; and call it on **both** buffers of a
    ...
    pub fn set_oe_window(&mut self, start: usize, lit: usize) {
        ...
        for plane in &mut self.planes {
            for row in &mut plane.rows {
                for i in 0..COLS {
                    let entry = &mut row.data[map_index(i)];
                    let bit = if i >= start && i < end {
                        OE_ACTIVE
                    } else {
                        OE_BLANK
                    };
                    entry.0 = (entry.0 & !OE_MASK) | bit;
                }
            }
        }
    }

    /// Writes one whole display row of already-quantised colour.
    ///
    /// Each component of `px` must already be in `0..(1 << PLANES)`; this is
    /// the output of the firmware's gamma/brightness LUT, so there is nothing
    /// left to scale. Hoisting the row lookup out of the per-pixel loop is
    /// the point: `set_pixel` re-indexes `planes[p].rows[r]` for every pixel
    /// and every plane, which on an ESP32 is most of the cost of converting a
    /// frame.
    ///
    /// Unlike [`Self::set_pixel`] this always writes, including black, so a
    /// full-frame conversion needs no [`Self::erase`] pass first.
    #[inline]
    pub fn write_row(&mut self, y: usize, px: &[[u8; 3]; COLS]) {
        if y >= NROWS * 2 {
            return;
        }
        ...
        for plane_idx in 0..PLANES {
            let row = &mut self.planes[plane_idx].rows[row_idx];
            for x in 0..COLS {
                let [r, g, b] = px[x];
                let bits = (((b >> plane_idx) & 1) << 2)
                    | (((g >> plane_idx) & 1) << 1)
                    | ((r >> plane_idx) & 1);
                let entry = &mut row.data[map_index(x)];
                if is_top {
    ...
    for DmaFrameBuffer<NROWS, COLS, PLANES>
{
    ...
    fn bcm_segment_ptr(&self, index: usize) -> *const u8 {
        ...
        (&raw const self.planes[index]).cast::<u8>()
    }
}
...
mod tests {
    ...
    fn format_sets_expected_row_addresses_for_all_rows() {
        ...
        for plane_idx in 0..8 {
            for slot in 0..16 {
                ...
                assert_eq!(
                    fb.planes[plane_idx].rows[slot].data[last_col].addr(),
                    u16::from(prev_addr)
                );
                assert_eq!(fb.planes[plane_idx].rows[slot].data[last_col].latch(), true);
            }
        }
    }
    ...
    fn set_pixel_maps_top_half_bits_per_plane() {
        ...
        for plane_idx in 0..8 {
            let bit = plane_idx;
            let entry = fb.planes[plane_idx].rows[map_row_index::<16>(3)].data[map_index(2)];
    ...
    fn set_pixel_maps_bottom_half_bits_per_plane() {
        ...
        for plane_idx in 0..8 {
            let bit = plane_idx;
            let entry = fb.planes[plane_idx].rows[map_row_index::<16>(4)].data[map_index(4)];
    ...
    fn reverse_row_order_stores_rows_back_to_front() {
        ...
        assert_eq!(fb.planes[0].rows[0].data[map_index(0)].addr(), 0);
        // The final slot renders panel row 0 and carries prev_addr 1.
        assert_eq!(fb.planes[0].rows[15].data[map_index(0)].addr(), 1);

        // Logical row 0 maps to the last memory slot.
        fb.set_pixel(Point::new(2, 0), Color::RED);
        let col2 = map_index(2);
        assert!(fb.planes[7].rows[15].data[col2].red1());
        assert!(!fb.planes[7].rows[0].data[col2].red1());
    }
    ...
    fn erase_clears_only_color_bits() {
        let mut fb = TestBuffer::new();
        let oe_before =
            fb.planes[0].rows[map_row_index::<16>(0)].data[map_index(1)].output_enable();
    ...
    fn draw_target_iter_sets_pixels() {
        ...
        for plane_idx in 0..8 {
            let bit = plane_idx;
            let entry = fb.planes[plane_idx].rows[map_row_index::<16>(1)].data[map_index(1)];
    ...
    fn test_skip_black_pixels_enabled() {
        ...
        assert!(fb.planes[0].rows[map_row_index::<16>(5)].data[mapped_col_10].red1());
        assert!(!fb.planes[0].rows[map_row_index::<16>(5)].data[mapped_col_10].grn1());
        assert!(!fb.planes[0].rows[map_row_index::<16>(5)].data[mapped_col_10].blu1());
    ...
    fn framebuffer_operations_trait_delegates_correctly() {
        ...
        assert!(fb.planes[0].rows[map_row_index::<16>(5)].data[map_index(3)].grn1());
